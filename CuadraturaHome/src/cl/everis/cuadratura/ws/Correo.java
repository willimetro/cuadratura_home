...
public class Correo {
	...
	private String getHtmlCorreo(Map<String, CountOBJ> c) {
		String toReturn = "<!DOCTYPE html>" + "<html>" + "<head>" + "<style>" + "table, th, td {"
				...
				+ "<p>Cinthya, a continuacion las tablas con Q para los siguientes cruces:</p>"
				//				+ "<br>"
				...
				&& null != c.get("ADICIONALES"))
						...
						+ ((null != c.get("INTERNET"))
								? "<td> -- </td>" : "")
						+ "</tr>" + "</table>"
						+ "<p><b>Adicionalmente se adjunta detalle de canales que aparecen solo en KALTURA  y "
						+ "no en 3PLAYFIBRA y tampoco en KENAN (archivo " + MessageFormat.format("adi_red_notplay_nokenan_{0}.csv", 
								LocalDateTime.now().format(DateTimeFormatter.ofPattern("ddMMyyyy"))) +") "
								+ " y tambien detalle de canales que aparecen solo en KENAN (archivo " + MessageFormat.format("adi_kenan_notplay_nored_{0}.csv", 
										LocalDateTime.now().format(DateTimeFormatter.ofPattern("ddMMyyyy"))) +") "
										+ "pero se debe considerar que incluye productos con ciclo de facturacion 62 y tambien con estado \"retirado\". "
										+ "Por último, recordar que los datos que se tienen de AAA son antiguos ya que en SPLUNK no se volvio a "
										+ "cargar esta informacion.<br>"
